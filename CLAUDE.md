...
ClioDeck is an Electron + React + TypeScript **desktop app for historians** covering the full research cycle: **explore → brainstorm → write → export**. Local-first, RAG-powered, with Zotero / Tropy / Obsidian integrations. Users: humanities researchers (history, DH). Licence **GPL-3.0-or-later**; current version **`1.0.0-rc.3`**.
...
- **No React component tests** — jsdom + `@testing-library/react` setup pending. (The 6 Brainstorm jsdom failures once listed here were fixed on 2026-07-18: a missing `window.electron.config` mock.)
...
- **The manuscript corpus has no UI**: the service, IPC channels and the `rag.indexManuscript` setting all exist, but nothing in the interface shows the index state or lets the user re-index.
