...
export function setupPDFHandlers() {
    ipcMain.handle('pdf:extractMetadata', async (_event, filePath) => {
    ...
    ipcMain.handle('pdf:index', async (event, filePath, bibtexKey, bibliographyMetadata) => {
        ...
        try {
            ...
            if (hm) {
                hm.logPDFOperation({
                    ...
                    citationsExtracted: document.citationsCount || 0,
                    durationMs,
                    metadata: {
                ...
                console.log(`📝 Logged PDF import: ${document.title} (${document.pageCount} pages, ${durationMs}ms)`);
            }
    ...
    ipcMain.handle('pdf:search', async (_event, query, options) => {
        try {
            const projectPath = projectManager.getCurrentProjectPath();
            requireProject(projectPath);
    ...
    ipcMain.handle('pdf:delete', async (_event, documentId) => {
        try {
            const projectPath = projectManager.getCurrentProjectPath();
            requireProject(projectPath);
    ...
    ipcMain.handle('pdf:get-document', async (_event, documentId) => {
