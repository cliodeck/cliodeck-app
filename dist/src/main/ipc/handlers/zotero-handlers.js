...
export function setupZoteroHandlers() {
    ipcMain.handle('zotero:test-connection', async (_event, userId, apiKey, groupId) => {
        ...
        try {
            ...
            const result = await zoteroService.testConnection(validatedData.userId, validatedData.apiKey, validatedData.groupId);
    ...
    ipcMain.handle('zotero:list-collections', async (_event, userId, apiKey, groupId) => {
        console.log('📞 IPC Call: zotero:list-collections', { userId, groupId });
        try {
            const validatedData = validate(ZoteroTestConnectionSchema, { userId, apiKey, groupId });
            const result = await zoteroService.listCollections(validatedData.userId, validatedData.apiKey, validatedData.groupId);
    ...
    ipcMain.handle('zotero:sync', async (_event, options) => {
                        ...
                        try {
                            ...
                            if (citations.length > 0) {
                                ...
                                const refreshResult = await zoteroService.refreshCollectionLinks({
                                    ...
                                    localCitations: citations.map((c) => ({
                                        id: c.id,
        ...
        try {
            ...
            if (result.success) {
                ...
                if (vectorStore) {
                    ...
                    const localCitations = originalCitations.map((c) => {
