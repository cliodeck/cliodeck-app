...
export function setupSimilarityHandlers() {
     ...
     */
    ipcMain.handle('similarity:analyze', async (event, text, options) => {
        ...
        try {
            ...
            const results = await similarityService.analyzeDocument(validatedData.text, validatedData.options, (progress) => {
                if (window) {
     ...
     */
    ipcMain.handle('similarity:get-segment-results', async (_event, segmentId) => {
