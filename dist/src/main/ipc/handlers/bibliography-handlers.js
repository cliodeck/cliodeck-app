...
export function setupBibliographyHandlers() {
    ipcMain.handle('bibliography:load', async (_event, filePath) => {
        try {
    ...
    ipcMain.handle('bibliography:parse', async (_event, content) => {
        try {
    ...
    ipcMain.handle('bibliography:search', async (_event, query) => {
        try {
    ...
    ipcMain.handle('bibliography:get-statistics', async (_event, citations) => {
    ...
    ipcMain.handle('bibliography:delete-orphan-pdfs', async (_event, filePaths) => {
    ...
    ipcMain.handle('bibliography:archive-orphan-pdfs', async (_event, options) => {
        ...
        try {
            const detector = new OrphanPDFDetector();
            const result = await detector.archiveOrphans(options.filePaths, options.projectPath, options.archiveSubdir);
    ...
    ipcMain.handle('bibliography:load-metadata', async (_event, projectPath) => {
