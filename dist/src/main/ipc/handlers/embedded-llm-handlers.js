...
import { ModelDownloader } from '../../../../backend/core/llm/ModelDownloader.js';
import { DEFAULT_EMBEDDED_MODEL } from '../../../../backend/core/llm/EmbeddedLLMClient.js';
...
export function setupEmbeddedLLMHandlers() {
    ...
    ipcMain.handle('embedded-llm:is-downloaded', async (_event, modelId) => {
    ...
    ipcMain.handle('embedded-llm:get-model-path', async (_event, modelId) => {
    ...
    ipcMain.handle('embedded-llm:get-model-info', async (_event, modelId) => {
    ...
    ipcMain.handle('embedded-llm:download', async (event, modelId) => {
        ...
        try {
            const modelPath = await dl.downloadModel(targetModelId, (progress) => {
            ...
            if (!modelLoadSuccess) {
                ...
                return errorResponse(new Error('Le modèle a été téléchargé mais n\'a pas pu être chargé (fichier corrompu). ' +
    ...
    ipcMain.handle('embedded-llm:delete-model', async (_event, modelId) => {
    ...
    ipcMain.handle('embedded-llm:set-provider', async (_event, provider) => {
