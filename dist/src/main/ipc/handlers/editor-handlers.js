...
export function setupEditorHandlers() {
    ipcMain.handle('editor:load-file', async (_event, filePath) => {
    ...
    ipcMain.handle('editor:save-file', async (_event, filePath, content, previousContent) => {
        ...
        try {
            ...
            if (hm) {
                ...
                const charactersDeleted = Math.max(0, (previousContent?.length || 0) - content.length);
                ...
                console.log(`📝 Logged document save: ${relativePath} (+${wordsAdded}w, -${wordsDeleted}w)`);
            }
    ...
    ipcMain.handle('editor:insert-text', async (event, text, metadata) => {
