...
export function setupTropyHandlers() {
     ...
     */
    ipcMain.handle('tropy:open-project', async (_event, tpyPath) => {
     ...
     */
    ipcMain.handle('tropy:sync', async (_event, options) => {
     ...
     */
    ipcMain.handle('tropy:start-watching', async (_event, tpyPath) => {
     ...
     */
    ipcMain.handle('tropy:perform-ocr', async (_event, imagePath, language) => {
     ...
     */
    ipcMain.handle('tropy:perform-batch-ocr', async (_event, imagePaths, language) => {
     ...
     */
    ipcMain.handle('tropy:import-transcription', async (_event, filePath, type) => {
        console.log('📞 IPC Call: tropy:import-transcription', { filePath, type });
        try {
            const result = await tropyService.importTranscription(filePath, type);
     ...
     */
    ipcMain.handle('tropy:get-source', async (_event, sourceId) => {
     ...
     */
    ipcMain.handle('tropy:update-transcription', async (_event, sourceId, transcription, source) => {
