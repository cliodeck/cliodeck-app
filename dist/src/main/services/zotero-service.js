 ...
 */
function generateBibtexKeyFromZoteroItem(item) {
...
class ZoteroService {
     ...
     */
    async testConnection(userId, apiKey, groupId) {
        try {
     ...
     */
    async listCollections(userId, apiKey, groupId) {
        try {
            const api = new ZoteroAPI({ userId, apiKey, groupId });
    ...
    sortCollectionsHierarchically(collections) {
        ...
        const addWithChildren = (parent, depth = 0) => {
            result.push(parent);
    ...
    async sync(options) {
        try {
            ...
            const bibtexKeyToCollections = {};
            // Filter to only bibliographic items (not attachments or notes)
            const bibliographicItems = result.items.filter((item) => item.data.itemType !== 'attachment' && item.data.itemType !== 'note');
    ...
    async enrichCitations(options) {
        try {
            ...
            const enrichedCitations = await sync.enrichCitationsWithAttachments(options.citations, bibliographicItems);
    ...
    async applyUpdates(options) {
        try {
            ...
            const sync = new ZoteroSync(api);
            // Apply updates
            const result = await sync.applyUpdates(options.currentCitations, options.diff, options.strategy, options.resolution);
            return {
                success: true,
