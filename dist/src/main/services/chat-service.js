...
class ChatService {
     ...
     */
    convertChunksToSummaries(searchResults) {
     ...
     */
    async getRelatedDocumentsFromGraph(documentIds, limit = 3) {
    ...
    async sendMessage(message, options = {}) {
        ...
        try {
            ...
            if (options.context) {
                ...
                if (searchResults.length > 0) {
                    ...
                    if (options.useGraphContext) {
                        ...
                        const relatedDocIds = await this.getRelatedDocumentsFromGraph(uniqueDocIds, options.additionalGraphDocs || 3);
            ...
            if (searchResults.length > 0) {
                ...
                const generator = llmProviderManager.generateWithSources(message, searchResults, projectContext, {
                    model: options.model,
                ...
                const generator = llmProviderManager.generateWithoutSources(message, [], {
                    model: options.model,
                    timeout: options.timeout,
                    generationOptions,
                    systemPrompt,
            ...
            if (hm) {
                ...
                if (options.context && searchResults.length > 0) {
                    ...
                    console.log(`📝 Logged RAG query: ${searchResults.length} sources, ${totalDuration}ms`);
                }
            }
