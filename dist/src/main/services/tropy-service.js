...
import { TropySync } from '../../../backend/integrations/tropy/TropySync.js';
import { TropyWatcher } from '../../../backend/integrations/tropy/TropyWatcher.js';
import { TropyOCRPipeline, SUPPORTED_OCR_LANGUAGES } from '../../../backend/integrations/tropy/TropyOCRPipeline.js';
import { PrimarySourcesVectorStore } from '../../../backend/core/vector-store/PrimarySourcesVectorStore.js';
import { createNERService } from '../../../backend/core/ner/NERService.js';
...
class TropyService {
    ...
    async sync(options) {
        ...
        const result = await this.tropySync.sync(this.currentTPYPath, this.vectorStore, syncOptions, onProgress);
     ...
     */
    async generateEmbeddingsForSources(onProgress) {
        if (!this.vectorStore) {
        ...
        for (let i = 0; i < sourcesToProcess.length; i++) {
            ...
            try {
                ...
                let fullText = '';
                if (source.title)
                    fullText += `Titre: ${source.title}\n\n`;
                if (source.creator)
                    fullText += `Auteur: ${source.creator}\n`;
                if (source.date)
                    fullText += `Date: ${source.date}\n`;
                if (source.archive)
                    fullText += `Archive: ${source.archive}\n`;
                if (source.collection)
                    fullText += `Collection: ${source.collection}\n`;
                fullText += '\n';
                if (source.transcription)
                    fullText += source.transcription;
     ...
     */
    chunkText(text, sourceId, sourceMeta) {
        if (!this.chunker) {
     ...
     */
    getRawChunks(text, sourceId, sourceMeta) {
        if (!this.chunker) {
            console.warn('⚠️ [TROPY-SERVICE] Chunker not initialized, using fallback');
            return [{
                    id: `${sourceId}-chunk-0`,
     ...
     */
    async performBatchOCR(imagePaths, language) {
        try {
            if (!this.ocrPipeline) {
                this.ocrPipeline = new TropyOCRPipeline();
            }
     ...
     */
    async importTranscription(filePath, type) {
        try {
            if (!this.ocrPipeline) {
                this.ocrPipeline = new TropyOCRPipeline();
            }
     ...
     */
    async expandQueryMultilingual(query, ollamaClient) {
     ...
     */
    async searchWithEntities(query, options) {
        if (!this.vectorStore) {
            console.warn('⚠️ [TROPY-SERVICE] VectorStore not initialized');
            return [];
        }
        ...
        try {
            ...
            const results = this.vectorStore.searchWithEntityBoost(queryEmbedding, queryEntities, topK * 2, expandedQuery);
     ...
     */
    searchWithEmbedding(queryEmbedding, topK = 10) {
        if (!this.vectorStore) {
            return [];
        }
     ...
     */
    async updateSourceTranscription(sourceId, transcription, source) {
        try {
            if (!this.vectorStore) {
