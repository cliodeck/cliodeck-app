...
import { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType, convertInchesToTwip, Table, TableRow, TableCell, WidthType, BorderStyle, ShadingType, UnderlineType, Header, Footer, PageNumber, } from 'docx';
 ...
 */
const unescapeCitations = (content) => {
    return content
...
export class WordExportService {
    constructor() {
        this.parser = new MarkdownToWordParser();
    }
    /**
     ...
     */
    async exportWithPandoc(options, outputPath, onProgress) {
     ...
     */
    async exportToWord(options, onProgress) {
        try {
            onProgress?.({
                stage: 'preparing',
            ...
            const outputPath = options.outputPath ||
                join(dirname(options.projectPath), `${options.metadata?.title || 'output'}.docx`);
            ...
            if (options.templatePath && existsSync(options.templatePath)) {
                ...
                try {
                    ...
                    finalBuffer = await this.mergeWithTemplate(options.templatePath, {
     ...
     */
    async mergeWithTemplate(templatePath, data) {
        try {
     ...
     */
    async findTemplate(projectPath) {
        try {
            const { readdir } = await import('fs/promises');
            const files = await readdir(projectPath);
            const templateFile = files.find((file) => extname(file).toLowerCase() === '.dotx');
            if (templateFile) {
