 ...
 */
function expandQueryMultilingual(query) {
...
class PDFService {
     ...
     */
    async init(projectPath, onRebuildProgress) {
        if (!projectPath) {
        ...
        try {
            ...
            this.ollamaClient = new OllamaClient(config.ollamaURL, config.ollamaChatModel, config.ollamaEmbeddingModel, config.embeddingStrategy || 'nomic-fallback');
            // Initialiser le LLM Provider Manager (gère Ollama + modèle embarqué)
            this.llmProviderManager = new LLMProviderManager({
                provider: config.generationProvider || 'auto',
            ...
            const useEnhancedSearch = ragConfig.useHNSWIndex !== false || ragConfig.useHybridSearch !== false;
            ...
            this.pdfIndexer = new PDFIndexer(this.vectorStore, this.ollamaClient, ragConfig.chunkingConfig, summarizerConfig, ragConfig.useAdaptiveChunking !== false, // Enable by default
    ...
    async indexPDF(filePath, bibtexKey, onProgress, bibliographyMetadata, collectionKeys) {
    ...
    async search(query, options) {
        ...
        if (sourceType === 'secondary' || sourceType === 'both') {
            ...
            allSourceResults.push(...secondaryResults.map((r) => ({
                ...r,
        ...
        if (sourceType === 'primary' || sourceType === 'both') {
            try {
                ...
                const mappedPrimaryResults = primaryResults.map((r) => ({
                    chunk: {
                        id: r.chunk?.id || r.id,
     ...
     */
    async searchSecondary(query, options) {
        const searchStart = Date.now();
        const ragConfig = configManager.getRAGConfig();
        const topK = options?.topK || ragConfig.topK;
        const threshold = options?.threshold || ragConfig.similarityThreshold;
        ...
        if (options?.collectionKeys && options.collectionKeys.length > 0) {
            const docsInCollections = this.vectorStore.getDocumentIdsInCollections(options.collectionKeys, true // recursive: include subcollections
            );
        ...
        const allResults = new Map(); // chunk.id → meilleur résultat
        ...
        const searchPromises = embeddings.map((queryEmbedding, i) => {
            ...
            if (this.vectorStore instanceof EnhancedVectorStore) {
                return this.vectorStore.search(expandedQuery, queryEmbedding, topK, documentIdsFilter);
            }
            else {
                return Promise.resolve(this.vectorStore.search(queryEmbedding, topK, documentIdsFilter));
            }
        });
     ...
     */
    async updateEmbeddedModel(modelPath, modelId) {
     ...
     */
    async getTextStatistics(options) {
        this.ensureInitialized();
        const documents = await this.vectorStore.getAllDocuments();
        ...
        const statistics = textometricsService.analyzeCorpus(corpusDocuments, options?.topN || 50);
        ...
        const wordFrequencyDistributionObj = {};
    ...
    async analyzeTopics(options) {
        ...
        const result = await topicModelingService.analyzeTopics(embeddings, texts, documentIds, analysisOptions);
