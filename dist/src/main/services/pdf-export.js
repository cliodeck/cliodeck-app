...
const getLatexTemplate = (projectType) => {
...
export class PDFExportService {
     ...
     */
    async exportToPDF(options, onProgress) {
        try {
            // Check dependencies
            ...
            if (options.projectType === 'presentation') {
                ...
                if (cfg.tocBeforeSection && cfg.showToc) {
                    beamerCustomizations.push('\\AtBeginSection[]{', '  \\begin{frame}<beamer>', '    \\frametitle{Plan}', '    \\tableofcontents[currentsection]', '  \\end{frame}', '}');
                }
