...
export class ZoteroDiffEngine {
     ...
     */
    async detectChanges(localCitations, remoteItems, options = {}) {
        const diff = {
            added: [],
            modified: [],
            deleted: [],
        ...
        for (const [key, localCitation] of localMap) {
            ...
            if (!zoteroKey)
                continue; // Skip local-only citations
            const remoteItem = remoteMap.get(zoteroKey);
            if (!remoteItem)
                continue; // Already handled in deleted
            const remoteCitation = this.zoteroItemToCitation(remoteItem);
     ...
     */
    compareCitations(local, remote, options) {
