...
import { TropyReader, } from './TropyReader.js';
import { TropyOCRPipeline } from './TropyOCRPipeline.js';
import { NERService } from '../../core/ner/NERService.js';
...
export class TropySync {
     ...
     */
    async sync(tpyPath, vectorStore, options, onProgress) {
     ...
     */
    async processItem(item, vectorStore, options) {
        const existingSource = vectorStore.getSourceByTropyId(item.id);
        let transcription = '';
        let transcriptionSource = undefined;
        let ocrCount = 0;
        ...
        if (options.transcriptionDirectory && (!transcription || options.forceReindex)) {
            const externalTranscription = await this.findExternalTranscription(item, options.transcriptionDirectory);
     ...
     */
    async findExternalTranscription(item, transcriptionDirectory) {
     ...
     */
    async performOCROnItem(item, language) {
    ...
    extractMetadata(item) {
        const metadata = {};
        if (item.template)
            metadata.template = item.template;
        if (item.type)
     ...
     */
    async extractEntitiesForSources(vectorStore, onProgress) {
        if (!this.nerService) {
     ...
     */
    checkSyncNeeded(projectPath, vectorStore) {
        const project = vectorStore.getTropyProject();
        if (!project)
            return true;
        if (project.tpyPath !== projectPath)
            return true;
        try {
...
// MARK: - Factory
export function createTropySync() {
