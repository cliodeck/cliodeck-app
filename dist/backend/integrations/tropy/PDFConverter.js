...
export class PDFConverter {
    constructor() {
        this.pdftoppmPath = null;
        this.pdftocairoPath = null;
    ...
    async initialize() {
        ...
        if (!this.pdftoppmPath && !this.pdftocairoPath) {
            throw new Error('Poppler utilities not found. Please install Poppler:\n' +
                '  macOS: brew install poppler\n' +
                '  Ubuntu: apt-get install poppler-utils\n' +
                '  Windows: Download from https://github.com/oschwartz10612/poppler-windows');
        }
     ...
     */
    async convertToTempFiles(pdfPath, options = {}) {
        await this.initialize();
        ...
        const tempDir = path.join(os.tmpdir(), `cliodeck-pdf-${Date.now()}`);
