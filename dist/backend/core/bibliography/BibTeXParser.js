...
export class BibTeXParser {
    ...
    parseFields(fieldsString) {
        ...
        while (currentIndex < fieldsString.length) {
            ...
            if (currentIndex >= fieldsString.length)
                break;
    ...
    createCitation(type, key, fields, bibDir) {
        // Champs obligatoires : author et title (year optionnel)
        const author = fields.author;
        const year = fields.year || fields.date || 'n.d.';
        const title = fields.title;
        if (!author || !title) {
            console.warn(`⚠️ Citation incomplète ignorée: ${key} (author=${!!author}, title=${!!title})`);
            return null;
        }
    ...
    resolveFilePath(fileField, bibDir) {
