...
export class KnowledgeGraphBuilder {
    ...
    async buildGraph(options = {}) {
        const { includeSimilarityEdges = true, similarityThreshold = 0.7, includeAuthorNodes = false, computeLayout = true, } = options;
     ...
     */
    addDocumentNodes(graph, documents) {
     ...
     */
    async addCitationEdges(graph, documents) {
        let citationCount = 0;
        for (const doc of documents) {
     ...
     */
    async addSimilarityEdges(graph, documents, threshold) {
     ...
     */
    addAuthorNodes(graph, documents) {
        // Agréger les auteurs
        const authors = new Map(); // auteur -> [docIds]
        for (const doc of documents) {
            if (doc.author) {
