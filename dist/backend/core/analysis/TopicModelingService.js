...
export class TopicModelingService {
    constructor() {
        this.serviceURL = 'http://127.0.0.1:8001';
        this.isStarting = false;
        this.isRunning = false;
        this.startupTimeout = 120000; // 120 secondes (chargement de torch/transformers peut être lent)
        this.autoStart = false; // Désactiver le démarrage automatique par défaut
    }
    /**
     ...
     */
    getVenvDir(isProduction, pythonServicePath) {
     ...
     */
    async setupVenv(venvDir, requirementsPath) {
        ...
        return new Promise((resolve, reject) => {
            ...
            pythonCheck.on('exit', (code) => {
                if (code === 0) {
                    ...
                    reject(new Error('Python is not available. Please install Python 3.11+ and required dependencies.'));
                }
            });
    ...
    async waitForServiceReady() {
        ...
        throw new Error(`Topic modeling service did not start within ${effectiveTimeout / 1000}s`);
    }
    /**
     ...
     */
    async analyzeTopics(embeddings, documents, documentIds, options = {}) {
        if (!this.isRunning) {
        ...
        if (embeddings.length < (options.minTopicSize || 5)) {
            throw new Error(`Not enough documents (${embeddings.length}). Minimum: ${options.minTopicSize || 5}`);
        }
     ...
     */
    async checkCriticalPackages(venvPython) {
        return new Promise((resolve) => {
            let resolved = false;
            let timeoutId;
     ...
     */
    async setupEnvironment(onProgress) {
        try {
            const log = (msg) => {
                console.log(msg);
                if (onProgress)
