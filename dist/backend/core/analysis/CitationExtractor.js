...
export class CitationExtractor {
     ...
     */
    extractCitations(fullText, pages) {
        const citations = [];
        const seenCitations = new Set(); // Pour éviter les doublons
     ...
     */
    extractInTextCitations(fullText, pages) {
        const citations = [];
     ...
     */
    extractBibliographyCitations(fullText) {
        const citations = [];
     ...
     */
    parseBibliographyEntries(bibliographyText) {
        const entries = [];
        ...
        for (const line of lines) {
            ...
            const isNewEntry = /^(?:[A-ZÀÂÄÆÇÉÈÊËÏÎÔŒÙÛÜ]|[\d]+\.|\[[\d]+\]|\•)/.test(trimmedLine);
     ...
     */
    extractAuthorFromBibliography(entry) {
     ...
     */
    extractContext(fullText, citationIndex) {
     ...
     */
    findPageNumber(position, fullText, pages) {
     ...
     */
    matchCitationsWithDocuments(citations, documents) {
     ...
     */
    findMatchingDocument(citation, documents) {
        ...
        for (const doc of documents) {
            ...
            if (docYear === citationYear) {
                if (docAuthor.includes(citationAuthor) ||
     ...
     */
    authorsAreSimilar(author1, author2) {
    ...
    getCitationStatistics(citations) {
        ...
        const allYears = citations.map((c) => c.year).filter(Boolean);
