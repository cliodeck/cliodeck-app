...
export class DocumentSummarizer {
    constructor(config, ollamaClient) {
     ...
     */
    async generateSummary(fullText, metadata) {
     ...
     */
    async generateSummaryEmbedding(summary) {
     ...
     */
    generateExtractiveSummary(fullText) {
     ...
     */
    scoreSentence(sentence, position, totalSentences, termFrequencies) {
        let score = 0;
        ...
        const keywordCount = this.academicKeywords.filter((keyword) => lowerSentence.includes(keyword)).length;
     ...
     */
    async generateAbstractiveSummary(fullText, metadata) {
        if (!this.ollamaClient) {
        ...
        const truncatedText = fullText.length > maxInputLength ? fullText.substring(0, maxInputLength) + '...' : fullText;
     ...
     */
    buildAbstractiveSummaryPrompt(text, metadata) {
