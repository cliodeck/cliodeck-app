...
import { DocumentChunker, CHUNKING_CONFIGS } from '../chunking/DocumentChunker.js';
...
import { DocumentSummarizer } from '../analysis/DocumentSummarizer.js';
export class PDFIndexer {
    constructor(vectorStore, ollamaClient, chunkingConfig = 'cpuOptimized', summarizerConfig, useAdaptiveChunking = false, ragConfig) {
        this.semanticChunker = null;
        this.documentSummarizer = null;
        ...
        if (this.options.useSemanticChunking) {
            ...
            this.semanticChunker = new SemanticChunker((text) => this.ollamaClient.generateEmbedding(text), {
     ...
     */
    async indexPDF(filePath, bibtexKey, onProgress, bibliographyMetadata) {
        ...
        try {
            ...
            if (citations.length > 0) {
                ...
                console.log(`   - ${stats.totalCitations} citations, ${stats.uniqueAuthors} auteurs, années ${stats.yearRange.min}-${stats.yearRange.max}`);
            }
            ...
            document.citationsExtracted = citations;
            document.summary = summary;
            document.summaryEmbedding = summaryEmbedding;
            ...
            const citationMatches = this.citationExtractor.matchCitationsWithDocuments(citations, allDocuments);
            ...
            console.log(`📊 Final: ${stats.totalChunks} chunks, ${stats.averageWordCount} mots/chunk en moyenne`);
            onProgress?.({
                stage: 'chunking',
                progress: 45,
            ...
            const baseStore = isEnhancedStore
                ...
                : this.vectorStore;
            const similaritiesCount = baseStore.computeAndSaveSimilarities(documentId, 0.5 // Seuil de similarité
            );
            onProgress?.({
                stage: 'completed',
                progress: 100,
     ...
     */
    async indexMultiplePDFs(filePaths, onProgress) {
        const documents = [];
