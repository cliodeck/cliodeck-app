...
import { entityNormalizer } from '../ner/EntityNormalizer.js';
...
export class PrimarySourcesVectorStore {
    constructor(projectPath) {
        ...
        this.hnswIndex = null;
        this.hnswDimension = 768;
        this.hnswMaxElements = 100000;
        this.hnswLabelMap = new Map(); // HNSW label -> chunk ID
        this.hnswCurrentSize = 0;
        this.hnswInitialized = false;
        this.bm25ChunkMap = new Map();
        this.bm25IdfCache = new Map();
        ...
        // HNSW parameters
        this.hnswM = 16;
        this.hnswEfConstruction = 100;
        this.hnswEfSearch = 50;
        // Hybrid search parameters
        this.rrfK = 60;
        this.denseWeight = 0.6;
        this.sparseWeight = 0.4;
        // MARK: - Entity-Boosted Search
        /**
         * Entity type weights for search scoring
         */
        this.entityTypeWeights = {
            PERSON: 1.5,
            EVENT: 1.4,
            DATE: 1.3,
            LOCATION: 1.2,
    ...
    rebuildBM25Index() {
        ...
        const rows = this.db.prepare('SELECT * FROM source_chunks').all();
        let docIndex = 0;
    ...
    saveSource(source) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(id, source.tropyId, source.title, source.date || null, source.creator || null, source.archive || null, source.collection || null, source.type || null, source.transcription || null, source.transcriptionSource || null, null, // language - à détecter plus tard
        source.lastModified.toISOString(), now, source.metadata ? JSON.stringify(source.metadata) : null);
     ...
     */
    getSource(id) {
        const row = this.db.prepare('SELECT * FROM primary_sources WHERE id = ?').get(id);
        if (!row)
            return null;
     ...
     */
    getAllSources() {
        const rows = this.db.prepare('SELECT * FROM primary_sources ORDER BY title').all();
    ...
    // MARK: - Photos
    saveSourcePhotos(sourceId, photos) {
    ...
    `);
        for (const photo of photos) {
            stmt.run(photo.id, sourceId, photo.path, photo.filename, photo.width || null, photo.height || null, photo.mimetype || null, photo.hasTranscription ? 1 : 0, photo.transcription || null);
        }
    }
    /**
     * Récupère les photos d'une source
     */
    getSourcePhotos(sourceId) {
        const rows = this.db
     ...
     */
    getAllTags() {
        const rows = this.db.prepare('SELECT DISTINCT tag FROM source_tags ORDER BY tag').all();
        return rows.map((row) => row.tag);
    }
    ...
    saveChunk(chunk, embedding) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
            .run(chunk.id, chunk.sourceId, chunk.content, chunk.chunkIndex, chunk.startPosition, chunk.endPosition, embeddingBuffer);
        // Add to HNSW index
     ...
     */
    addToHNSWIndex(chunkId, embedding) {
        if (!this.hnswInitialized || !this.hnswIndex) {
            this.initNewHNSWIndex();
        }
    ...
    `);
        const transaction = this.db.transaction(() => {
            for (const { chunk, embedding } of chunks) {
                const embeddingBuffer = Buffer.from(embedding.buffer);
                stmt.run(chunk.id, chunk.sourceId, chunk.content, chunk.chunkIndex, chunk.startPosition, chunk.endPosition, embeddingBuffer);
            }
        });
        transaction();
    }
     ...
     */
    search(queryEmbedding, topK = 10, query) {
        const startTime = Date.now();
        // DEBUG: Log index state at search time
        const dbChunkCount = this.db.prepare('SELECT COUNT(*) as count FROM source_chunks WHERE embedding IS NOT NULL').get().count;
    ...
    searchHNSW(queryEmbedding, k) {
        ...
        for (let i = 0; i < result.neighbors.length; i++) {
            ...
            const chunkRow = this.db.prepare('SELECT * FROM source_chunks WHERE id = ?').get(chunkId);
            if (!chunkRow)
                continue;
            const chunk = {
                id: chunkRow.id,
    ...
    rebuildAllIndexes() {
        ...
        if (chunks.length === 0) {
            ...
            const totalChunks = this.db.prepare('SELECT COUNT(*) as count FROM source_chunks').get().count;
            const chunksWithEmbedding = this.db.prepare('SELECT COUNT(*) as count FROM source_chunks WHERE embedding IS NOT NULL').get().count;
     ...
     */
    getStatistics() {
        const sourceCount = this.db.prepare('SELECT COUNT(*) as count FROM primary_sources').get().count;
        const chunkCount = this.db.prepare('SELECT COUNT(*) as count FROM source_chunks').get().count;
        const photoCount = this.db.prepare('SELECT COUNT(*) as count FROM source_photos').get().count;
        const withTranscription = this.db
            .prepare('SELECT COUNT(*) as count FROM primary_sources WHERE transcription IS NOT NULL')
            .get().count;
        // Compter par archive
        const archiveRows = this.db
            .prepare('SELECT archive, COUNT(*) as count FROM primary_sources WHERE archive IS NOT NULL GROUP BY archive')
            .all();
        const byArchive = {};
        ...
        const collectionRows = this.db
            .prepare('SELECT collection, COUNT(*) as count FROM primary_sources WHERE collection IS NOT NULL GROUP BY collection')
            .all();
        const byCollection = {};
     ...
     */
    saveTropyProject(tpyPath, name, autoSync = false) {
        const id = randomUUID();
        const now = new Date().toISOString();
        this.db
            .prepare(`
     ...
     */
    getTropyProject() {
        const row = this.db.prepare('SELECT * FROM tropy_projects LIMIT 1').get();
        if (!row)
            return null;
        return {
            id: row.id,
     ...
     */
    saveEntity(entity) {
        const normalizedName = entityNormalizer.normalize(entity.name, entity.type);
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `)
            .run(id, entity.name, entity.type, normalizedName, entity.aliases ? JSON.stringify(entity.aliases) : null, now);
        return id;
    }
    /**
     * Saves an entity mention
     */
    saveEntityMention(mention) {
        const id = randomUUID();
        this.db
            .prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `)
            .run(id, mention.entityId, mention.chunkId || null, mention.sourceId, mention.startPosition || null, mention.endPosition || null, mention.context || null);
    }
    /**
     * Updates or creates a relation between two entities
     */
    updateEntityRelation(entity1Id, entity2Id, sourceId) {
        ...
        if (existing) {
            ...
            const sourceIds = JSON.parse(existing.source_ids || '[]');
     ...
     */
    saveEntitiesForSource(sourceId, entities, chunkId) {
     ...
     */
    getChunkIdsWithEntity(entityId) {
        const rows = this.db
     ...
     */
    getRelatedEntities(entityId, limit = 10) {
        const rows = this.db
            .prepare(`
        SELECT e.*, er.weight
        FROM entities e
     ...
     */
    deleteEntitiesForSource(sourceId) {
    ...
    getEntityStatistics() {
        ...
        const totalMentions = this.db.prepare('SELECT COUNT(*) as count FROM entity_mentions').get().count;
        const totalRelations = this.db.prepare('SELECT COUNT(*) as count FROM entity_relations').get().count;
     ...
     */
    searchWithEntityBoost(queryEmbedding, queryEntities, topK = 10, query, hybridWeight = 0.7, entityWeight = 0.3) {
        ...
        const matchingEntityIds = new Map();
        ...
        const chunkEntityScores = new Map();
        ...
        const boostedResults = baseResults.map(result => {
            ...
            const combinedScore = hybridWeight * result.similarity +
        ...
        console.log(`🏷️ [ENTITY-BOOST] Boosted ${boostedResults.filter(r => r.entityScore > 0).length} results with entity matches`);
