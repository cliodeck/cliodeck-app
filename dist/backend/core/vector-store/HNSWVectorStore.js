...
export class HNSWVectorStore {
    constructor(projectPath, dimension = 768, maxElements = 100000) {
        this.index = null;
        this.isInitialized = false;
        this.currentSize = 0;
        this.wasCorrupted = false; // Track if corruption was detected
        // HNSW parameters
        this.M = 16; // Number of connections per layer (default: 16)
        this.efConstruction = 100; // Construction-time accuracy (default: 200, reduced for RAM)
        this.efSearch = 50; // Search-time accuracy (default: 50)
    ...
    async addChunk(chunk, embedding) {
        ...
        if (embedding.length !== this.dimension) {
            throw new Error(`Embedding dimension mismatch: expected ${this.dimension}, but got ${embedding.length}. ` +
    ...
    async addChunks(chunks) {
        ...
        for (let idx = 0; idx < chunks.length; idx++) {
            ...
            // Validate embedding dimension
            if (embedding.length !== this.dimension) {
                console.warn(`⚠️  Skipping chunk ${chunk.id}: dimension mismatch (expected ${this.dimension}, got ${embedding.length})`);
                skippedCount++;
                continue;
            }
            // Validate embedding values (check for NaN/Infinity which can crash native code)
     ...
     */
    async search(queryEmbedding, k = 10, documentIds) {
        if (!this.isInitialized || !this.index) {
        ...
        for (let i = 0; i < result.neighbors.length; i++) {
            ...
            searchResults.push({
                chunk,
                similarity,
                document: null, // Will be populated by VectorStore wrapper
            });
    ...
    async loadMetadata() {
        ...
        try {
            ...
            if (metadata.dimension && metadata.dimension !== this.dimension) {
                console.warn(`⚠️  Metadata dimension mismatch: file has ${metadata.dimension}, expected ${this.dimension}`);
                this.wasCorrupted = true;
                fs.unlinkSync(metadataPath);
                return false;
            }
