...
import { CHUNKING_CONFIGS } from './DocumentChunker.js';
/**
...
export class AdaptiveChunker {
    constructor(config = CHUNKING_CONFIGS.cpuOptimized) {
        this.config = config;
    }
    /**
     * Create chunks using adaptive structure-aware strategy
     */
    createChunks(pages, documentId, documentMeta) {
        // Combine all pages
        ...
        for (const section of sections) {
            ...
            const sectionChunks = this.chunkSection(section, documentId, chunkIndex, pageMapping, documentMeta);
            chunks.push(...sectionChunks);
            chunkIndex += sectionChunks.length;
        }
        console.log(`✅ ${chunks.length} adaptive chunks created (${sections.length} sections detected)`);
        return chunks;
    }
    /**
     ...
     */
    chunkSection(section, documentId, startingIndex, pageMapping, documentMeta) {
        ...
        if (words.length <= this.config.maxChunkSize) {
            ...
            const content = this.enhanceChunkWithContext(this.cleanText(section.content), documentMeta, section.title);
            return [
                {
                    id: randomUUID(),
                    documentId,
        ...
        for (const paragraph of paragraphs) {
            ...
            if (currentWords.length + paragraphWords.length > this.config.maxChunkSize &&
                ...
                const content = this.enhanceChunkWithContext(this.cleanText(finalChunk), documentMeta, section.title);
                chunks.push({
                    id: randomUUID(),
                    documentId,
                    content,
     ...
     */
    findPageNumber(position, mapping) {
     ...
     */
    createSmartOverlap(text, targetWords) {
     ...
     */
    enhanceChunkWithContext(content, documentMeta, sectionTitle) {
