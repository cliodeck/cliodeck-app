...
export class ChunkDeduplicator {
    ...
    deduplicate(chunks, options) {
        ...
        if (config.useSimilarity) {
            const simResult = this.deduplicateBySimilarity(result, config.similarityThreshold);
     ...
     */
    deduplicateBySimilarity(chunks, threshold) {
        const uniqueChunks = [];
        const duplicateMap = new Map();
     ...
     */
    isNearDuplicate(content, existingContents, threshold = 0.85) {
