...
export class SemanticChunker {
    constructor(generateEmbedding, config = {}, cache) {
     ...
     */
    async createChunks(pages, documentId, documentMeta) {
        // Combine all text
        ...
        const chunks = this.createChunksFromBoundaries(sentences, boundaries, documentId, pages, documentMeta);
        // Log cache stats
        const stats = this.cache.getStats();
        console.log(`📊 [SEMANTIC] Cache stats: ${stats.hits} hits, ${stats.misses} misses (${(stats.hitRate * 100).toFixed(1)}% hit rate)`);
        return chunks;
    }
    /**
     * Detect semantic boundaries between sentences
     */
    async detectBoundaries(sentences) {
        ...
        const threshold = Math.min(this.config.similarityThreshold, avgSimilarity - 0.1);
     ...
     */
    createChunksFromBoundaries(sentences, boundaries, documentId, pages, documentMeta) {
        const chunks = [];
        ...
        for (let i = 0; i < boundaryPositions.length - 1; i++) {
            ...
            if (wordCount > this.config.maxChunkSize) {
                const subChunks = this.splitLargeChunk(chunkSentences, documentId, chunkIndex, pageMapping, documentMeta);
    ...
    splitIntoSentences(text) {
        ...
        for (const abbrev of abbrevs) {
            processed = processed.replace(new RegExp(`\\b${abbrev}\\.`, 'gi'), `${abbrev}<<<DOT>>>`);
        }
     ...
     */
    async generateWindowEmbeddings(windows) {
        const embeddings = [];
        for (const window of windows) {
            const embedding = await this.cache.getOrCompute(window, this.generateEmbedding);
     ...
     */
    filterCloseBoundaries(boundaries, minDistance) {
     ...
     */
    splitLargeChunk(sentences, documentId, startIndex, pageMapping, documentMeta) {
        const chunks = [];
