...
import { getDefaultSystemPrompt } from './SystemPrompts.js';
 ...
 */
export function getModelContextInfo(modelName) {
...
export function classifyOllamaError(error, context) {
    ...
    const formatContext = () => {
        ...
        if (context?.promptLength)
            parts.push(`Prompt: ${Math.round(context.promptLength / 1000)}k caractères`);
        if (context?.sourceCount)
            parts.push(`Sources: ${context.sourceCount}`);
        if (context?.numCtx)
            parts.push(`Contexte LLM: ${context.numCtx} tokens`);
        if (context?.model)
            parts.push(`Modèle: ${context.model}`);
        return parts.join(', ');
    };
    ...
    if (messageLower.includes('context length') ||
    ...
    if (messageLower.includes('econnrefused') ||
...
export class OllamaClient {
    constructor(baseURL = 'http://127.0.0.1:11434', chatModel, embeddingModel, embeddingStrategy) {
        this.embeddingModel = 'nomic-embed-text';
        this.chatModel = 'gemma2:2b';
        this.embeddingStrategy = 'nomic-fallback';
     ...
     */
    async generateEmbeddingWithModel(text, model) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Ollama embedding error: ${response.status} ${response.statusText} - ${errorBody}`);
        }
     ...
     */
    async generateEmbeddingForChunk(text) {
    ...
    async *generateResponseStream(prompt, context, modelOverride, timeoutOverride, generationOptions, systemPrompt) {
        const url = `${this.baseURL}/api/generate`;
        ...
        if (!response.ok || !response.body) {
            ...
            const classified = classifyOllamaError(new Error(`${response.status} ${response.statusText}: ${errorBody}`), errorContext);
            const enhancedError = new Error(`${classified.userMessage}\n\n💡 ${classified.suggestion}`);
            enhancedError.classified = classified;
            throw enhancedError;
        }
    ...
    async *generateResponseStreamWithSources(prompt, sources, projectContext, modelOverride, timeoutOverride, generationOptions, systemPrompt) {
        const url = `${this.baseURL}/api/generate`;
...
Réponds de manière précise et académique en te basant sur les extraits fournis. Si les extraits ne contiennent pas l'information nécessaire, dis-le clairement. Cite les numéros des extraits que tu utilises.`;
    ...
    buildPromptWithSources(userQuery, sources, projectContext, systemPrompt) {
        // Use provided system prompt, or fallback to French default for backwards compatibility
        const systemInstruction = systemPrompt || getDefaultSystemPrompt('fr');
        if (sources.length === 0) {
...
Liste des sources disponibles :`;
    ...
    inferRecommendations(modelName) {
