...
import fs from 'fs';
import { EMBEDDED_MODELS, DEFAULT_EMBEDDED_MODEL, } from './EmbeddedLLMClient.js';
...
export class ModelDownloader {
     ...
     */
    getModelPath(modelId = DEFAULT_EMBEDDED_MODEL) {
     ...
     */
    isModelDownloaded(modelId = DEFAULT_EMBEDDED_MODEL) {
        try {
            ...
            if (!sizeValid) {
                console.warn(`⚠️ [DOWNLOAD] Model ${modelId} exists but size mismatch: ${sizeMB.toFixed(1)} MB vs expected ≥${(modelInfo.sizeMB * 0.98).toFixed(1)} MB`);
                return false;
            }
     ...
     */
    deleteCorruptedModel(modelId = DEFAULT_EMBEDDED_MODEL) {
        try {
            const modelPath = this.getModelPath(modelId);
            if (fs.existsSync(modelPath)) {
     ...
     */
    getModelInfo(modelId = DEFAULT_EMBEDDED_MODEL) {
     ...
     */
    async downloadModel(modelId = DEFAULT_EMBEDDED_MODEL, onProgress) {
        ...
        try {
            ...
            if (downloadedSize < totalSize * 0.99) {
                ...
                if (fs.existsSync(destPath))
                    fs.unlinkSync(destPath);
                throw new Error(`Download stream ended prematurely: received ${(downloadedSize / (1024 * 1024)).toFixed(1)} MB ` +
            ...
            if (stats.size < contentLengthMinSize) {
                fs.unlinkSync(destPath);
                throw new Error(`Téléchargement incomplet: ${actualSizeMB.toFixed(1)} MB reçus sur ${contentLengthMB.toFixed(1)} MB attendus (Content-Length). ` +
            ...
            if (stats.size < expectedMinSize) {
                fs.unlinkSync(destPath);
                throw new Error(`Fichier trop petit: ${actualSizeMB.toFixed(1)} MB au lieu de ${expectedSizeMB} MB minimum attendu. ` +
     ...
     */
    deleteModel(modelId = DEFAULT_EMBEDDED_MODEL) {
        try {
            const modelPath = this.getModelPath(modelId);
            if (fs.existsSync(modelPath)) {
                fs.unlinkSync(modelPath);
