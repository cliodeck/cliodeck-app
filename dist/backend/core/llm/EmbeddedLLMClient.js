...
export class EmbeddedLLMClient {
    constructor() {
        ...
        this.model = null;
        this.context = null;
        this.sequence = null; // Store the sequence for reuse
        this.modelPath = null;
     ...
     */
    async initialize(modelPath, modelId) {
        try {
...
`;
     ...
     */
    async *generateResponseStreamWithSources(prompt, sources, projectContext, systemPrompt) {
     ...
     */
    async *generateResponseStream(prompt, context, systemPrompt) {
        // Convertir le contexte simple en format utilisable
        yield* this.generateResponseStreamWithSources(prompt, [], context.join('\n'), systemPrompt);
    }
    /**
