...
export class HistoryManager {
    ...
    migrateDatabase() {
        ...
        if (currentVersion < 2) {
            try {
                ...
                const chatColumns = this.db.pragma('table_info(chat_messages)');
                if (!chatColumns.some((c) => c.name === 'mode_id')) {
                    this.db.exec('ALTER TABLE chat_messages ADD COLUMN mode_id TEXT');
                    console.log('📝 Migration v2: Added mode_id to chat_messages');
                }
                const aiColumns = this.db.pragma('table_info(ai_operations)');
                if (!aiColumns.some((c) => c.name === 'mode_id')) {
    ...
    startSession(metadata) {
      ...
      VALUES (?, ?, ?, ?)
    `);
        stmt.run(sessionId, this.projectPath, now, metadata ? JSON.stringify(metadata) : null);
    ...
    logEvent(eventType, eventData) {
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
        stmt.run(eventId, this.currentSessionId, eventType, now, eventData ? JSON.stringify(eventData) : null);
    ...
    `);
        stmt.run(opId, this.currentSessionId, operation.operationType, now, operation.durationMs || null, operation.inputText || null, operation.inputMetadata ? JSON.stringify(operation.inputMetadata) : null, operation.modelName || null, operation.modelParameters ? JSON.stringify(operation.modelParameters) : null, operation.outputText || null, operation.outputMetadata ? JSON.stringify(operation.outputMetadata) : null, operation.success ? 1 : 0, operation.errorMessage || null);
    ...
    // Document Operations
    // ==========================================================================
    logDocumentOperation(operation) {
        if (!this.currentSessionId)
            return '';
        const opId = randomUUID();
        const now = new Date().toISOString();
    ...
    `);
        stmt.run(opId, this.currentSessionId, operation.operationType, operation.filePath, now, operation.wordsAdded || null, operation.wordsDeleted || null, operation.charactersAdded || null, operation.charactersDeleted || null, operation.contentHash || null);
    ...
    `);
        stmt.run(opId, this.currentSessionId, operation.operationType, operation.documentId || null, now, operation.durationMs || null, operation.filePath || null, operation.pageCount || null, operation.chunksCreated || null, operation.citationsExtracted || null, operation.metadata ? JSON.stringify(operation.metadata) : null);
    ...
    logChatMessage(message) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(msgId, this.currentSessionId, message.role, message.content, message.sources ? JSON.stringify(message.sources) : null, now, modeId);
        return msgId;
    }
    // ==========================================================================
    // Query Methods
    ...
    getChatMessagesForSession(sessionId) {
        const stmt = this.db.prepare(`
    ...
    getAIOperationsForSession(sessionId) {
        const stmt = this.db.prepare(`
    ...
    getDocumentOperationsForSession(sessionId) {
        const stmt = this.db.prepare(`
    ...
    getPDFOperationsForSession(sessionId) {
        const stmt = this.db.prepare(`
    ...
    // Export Methods
    // ==========================================================================
    exportSessionReport(sessionId, format) {
    ...
    exportAsMarkdown(session, events, chatMessages, aiOps, docOps, pdfOps) {
        ...
        if (pdfOps.length > 0) {
            ...
            pdfOps.forEach((op) => {
                ...
                if (op.pageCount)
                    md += `- **Pages:** ${op.pageCount}\n`;
                if (op.chunksCreated)
                    md += `- **Chunks Created:** ${op.chunksCreated}\n`;
                if (op.citationsExtracted)
                    md += `- **Citations Extracted:** ${op.citationsExtracted}\n`;
                if (op.durationMs)
        ...
        if (docOps.length > 0) {
            ...
            docOps.forEach((op) => {
                ...
                md += `- **Time:** ${op.timestamp.toLocaleString()}\n`;
                if (op.wordsAdded)
                    md += `- **Words Added:** ${op.wordsAdded}\n`;
                if (op.wordsDeleted)
                    md += `- **Words Deleted:** ${op.wordsDeleted}\n`;
                md += `\n`;
            });
        }
        // AI Operations
        ...
        if (chatMessages.length > 0) {
            ...
            chatMessages.forEach((msg) => {
                ...
                if (msg.sources && msg.sources.length > 0) {
                    md += `*Sources:* ${msg.sources.map((s) => s.documentTitle || s.title).join(', ')}\n\n`;
                }
            });
        }
        return md;
    }
    exportAsLaTeX(session, events, chatMessages, aiOps, docOps, pdfOps) {
        ...
        if (docOps.length > 0) {
            ...
            docOps.forEach((op) => {
                ...
                if (op.wordsAdded)
                    latex += ` - +${op.wordsAdded} words`;
                if (op.wordsDeleted)
                    latex += ` -${op.wordsDeleted} words`;
                latex += `\n`;
            });
            latex += `\\end{itemize}\n\n`;
        }
    ...
    getStatistics() {
        const stats = {
            ...
            totalEvents: this.db.prepare('SELECT COUNT(*) as count FROM events').get()
                .count,
            totalChatMessages: this.db.prepare('SELECT COUNT(*) as count FROM chat_messages').get().count,
            totalAIOperations: this.db.prepare('SELECT COUNT(*) as count FROM ai_operations').get().count,
            averageSessionDuration: this.db
                .prepare('SELECT AVG(total_duration_ms) as avg FROM sessions WHERE total_duration_ms IS NOT NULL')
                .get().avg || 0,
        };
        return stats;
    }
