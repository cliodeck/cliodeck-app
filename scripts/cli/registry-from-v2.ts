/**
 * Build a ProviderRegistry from a workspace config (fusion step 4.6).
 *
 * The CLI runs outside Electron, so it can't reach the user-level
 * config-manager. Instead we read the `.cliodeck/config.json` whose
 * `llm`/`embedding` sections match the registry config shape directly.
 *
