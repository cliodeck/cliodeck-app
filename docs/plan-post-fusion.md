...
| 1.8 | Backend | M | 🔬 Recherche documentaire faite — voir `docs/research-ollama-tools-1.8.md`. Whitelist proposée : `mistral-nemo`, `qwen3:8b`, `qwen3:14b`, `mistral-small3.2`, `qwen2.5:14b`, `qwen3:32b`. **Llama 3.1 retiré** (correspond exactement au bug décrit par le mainteneur). Reste à implémenter : map `model → tools-capable`, normaliseur `arguments` (object vs string, issue #6002), détection version Ollama pour streaming tools. | Oui — décider whitelist finale + implémenter ([A9](actions-frederic.md#a9)) |
