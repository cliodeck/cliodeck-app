...
| -- | Backend | **Closed in rc.6 (re-measured 2026-09-17).** The 144 chunks without embeddings (5880 indexed, 5736 with embeddings) observed in May 2026 are gone: `npm run project:health` on the real project reports 1268 chunks, **0 without embedding**, HNSW aligned (1268 vectors). The May figure matches the duplicated corpus the rc.5 deduplication removed (5920 chunks for 55 PDFs) | No |
...
| 18 | Security | **Done in rc.6** (branch `fix/fusion-ipc-validation`). The count was misleading: of 35 handlers, 18 take renderer input and most already guarded it by hand. Five real gaps, each now tested: `vault:set-path` accepted any folder (now only one chosen in the native dialog, `isConsentedPath`) — a compromised renderer could point the vault at `~` and pull `~/.ssh` into a corpus the chat can send out; `vault:import-as-ideas` never used its schema (whose fields matched nothing the renderer sent), so a non-numeric `maxFiles` lifted the bound; `consent:grant` stored the renderer's label although the decision compares it (now granted only for the provider main will call); `recipes:run` cast `inputs` blindly; `security:get-events` had no upper bound | No |
