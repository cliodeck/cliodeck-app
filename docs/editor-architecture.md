...
| `src/editor/lezer-extensions/` | Extensions de parsing `Footnotes` et `PandocCitations` (`MarkdownConfig` officiel). Zéro import ClioDeck, doc en anglais. **Empaquetées** dans `packages/lezer-footnotes/` et `packages/lezer-pandoc-citations/` (v0.1.0, licence MIT — l'exception assumée au GPLv3 du dépôt, ces extensions n'ayant de valeur que réutilisables) ; **pas encore publiées sur npm**. |
