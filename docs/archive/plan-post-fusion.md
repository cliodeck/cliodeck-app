...
| 2.5 | Frontend | M | ✅ Tool-use MCP exposé dans Brainstorm. Bandeau compact `McpToolsBanner` au-dessus de la zone de message (caché tant qu'aucun client MCP n'est `ready`), heuristique `classifyMcpTool` lecture/écriture, store `mcpToolsStore` persiste les opt-in/opt-out. Politique A12 (c) appliquée : lecture seule activée par défaut, écriture sur consentement. `chat:start` reçoit un `enabledTools?: string[]` qui filtre le catalogue côté backend. 59 tests dédiés. | Fait |
| 2.6 | Frontend | M | ✅ Pont Brainstorm → Write — insertion à la position du curseur (A13 option a). `editorStore.insertDraftAtCursor` gère Monaco (offset précis), Milkdown (approximation via `textBetween`), et fallback append quand aucun éditeur n'est monté. L'undo natif des deux éditeurs (Cmd/Ctrl-Z) défait le splice. Le panneau « Brouillons » (option c) est traqué dans une issue séparée. | Fait |
...
| 2.8 | Frontend + Design | M | ✅ Panneau « Événements détectés » dans `SecurityConfigSection` — total, répartition par `kind` et `severity`, derniers événements (20). Reader pur `events-reader.ts` (parse JSONL fail-soft + agrégation), IPC `fusion:security:get-events`. Le badge sur les sources flaggées dans le chat (deuxième volet de A15) reste à faire — ouvert dans une issue follow-up. | Fait |
...
| 3.3 | Design | S | ✅ `*:focus-visible` global avec ring `--color-accent` (2px outline + 2px offset) — clavier-only ; skip-link première position du DOM (caché jusqu'au focus) qui saute à `#main-content` ; nouveau namespace i18n `a11y.*`. **Contraste icônes (`strokeWidth={1}` × 73)** non touché — décision aesthétique. | Fait (focus + skip-link) |
...
| 3.11 | Backend | L | ✅ `SecondaryRetriever` extrait dans `backend/core/rag/retrievers/secondary-retriever.ts`. Pipeline complet (filter resolution / expansion / embedding fan-out / hybrid HNSW+BM25 / threshold + cross-language fallback) hors de `retrieval-service.ts`. `searchSecondary` devient un délégué de ~30 LOC. `ACADEMIC_TERMS_FR_TO_EN` source unique dans le retriever, ré-importée par le warmup. 14 tests couvrant store × variant × filtres × fallback. | Fait |
