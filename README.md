...
- **Linux** — AppImage and `.deb` for **x86_64** (`uname -m` prints `x86_64`); no arm64 build
...
- **MCP server (inactive by default)** — expose your corpus to Claude Desktop / Cursor over stdio with a typed, auditable JSONL access log. Each tool is granted or withheld one by one; a withheld tool is not registered at all, so the client never sees its name. Reading your own manuscript (`list_manuscript` / `read_manuscript`) is the one capability that stays off until you grant it. One-click install for Claude Desktop (a `.mcpb` extension) or a named entry per project in its configuration file; on Linux, where Claude Desktop does not exist, a `claude mcp add` command for Claude Code.
